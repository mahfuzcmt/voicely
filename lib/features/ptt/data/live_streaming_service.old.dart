...
class LiveStreamingService {
  ...
  // Minimum broadcast duration to ensure audio establishes before release
  ...
  /// Stream of audio renderer updates for UI consumption
  /// In Flutter WebRTC, audio renderers need to be attached to RTCVideoView widgets for audio to play
  final _audioRendererController = StreamController<Map<String, RTCVideoRenderer>>.broadcast();
  Stream<Map<String, RTCVideoRenderer>> get audioRendererStream => _audioRendererController.stream;

  /// Get current audio renderers snapshot for initial UI setup
  ...
  Future<void> forceStopAllConnections() async {
    ...
    // CRITICAL: Dispose local stream to free native resources on low-end devices
    ...
    _connectionSubscription = _wsService.connectionState.listen((connState) {
      if (connState == WSConnectionState.authenticated) {
        ...
        if (_state == LiveStreamingState.broadcasting ||
          ...
          // CRITICAL: Dispose local stream if we were broadcasting to free resources
          if (wasBroadcasting) {
            _disposeLocalStream();
          }
          _updateState(LiveStreamingState.idle);
  ...
  Future<void> stopBroadcasting() async {
    ...
    // CRITICAL: Enforce minimum broadcast duration to ensure ICE connection
    // can establish and audio has time to be transmitted before releasing floor.
    // Without this, quick button releases would result in no audio being heard.
    if (_broadcastStartTime != null) {
      final elapsed = DateTime.now().difference(_broadcastStartTime!);
      if (elapsed < _minBroadcastDuration) {
        final waitTime = _minBroadcastDuration - elapsed;
        debugPrint('LiveStream: Waiting ${waitTime.inMilliseconds}ms to meet minimum broadcast duration');
    ...
    // CRITICAL FIX: Dispose local stream to free native resources
    // On low-end devices like T310, keeping the stream alive causes memory pressure
    // and eventual crashes after 2-3 broadcasts. The stream will be re-initialized
    // on the next broadcast (slightly slower first PTT, but prevents crashes).
    await _disposeLocalStream();
  ...
  /// Dispose the local stream to free native resources
  /// Called after stopping broadcast to prevent memory accumulation on low-end devices
  Future<void> _disposeLocalStream() async {
    final stream = _localStream;
    if (stream == null) return;

    debugPrint('LiveStream: Disposing local stream to free native resources');
    try {
      ...
      for (final track in stream.getTracks()) {
        try {
          ...
          debugPrint('LiveStream: Error stopping track ${track.id}: $e');
        }
      }
      // Dispose the stream
      await stream.dispose();
      _localStream = null;
      debugPrint('LiveStream: Local stream disposed successfully');
    } catch (e) {
      debugPrint('LiveStream: Error disposing local stream: $e');
      _localStream = null; // Clear reference even on error
    }
  }
  ...
  Future<void> _createAudioRenderer(String peerId, MediaStream stream) async {
    try {
      ...
      // Notify UI that a new renderer is available for consumption
      // This is CRITICAL for audio playback - the renderer MUST be in an RTCVideoView
      _audioRendererController.add(Map.unmodifiable(_audioRenderers));
      debugPrint('LiveStream: Renderer update emitted to UI');
    } catch (e) {
  ...
  Future<void> _closePeerConnection(String peerId) async {
    ...
    // Dispose the audio renderer - wrap in try-catch to prevent crashes
    ...
    if (renderer != null) {
      try {
        ...
        // Notify UI that renderer was removed
        _audioRendererController.add(Map.unmodifiable(_audioRenderers));
      } catch (e) {
        // Ignore disposal errors - the renderer may already be in invalid state
        debugPrint('LiveStream: Renderer disposal error (ignored): $e');
      }
    }
  ...
  Future<void> dispose() async {
    ...
    // Dispose all audio renderers - wrap in try-catch to prevent crashes
    for (final renderer in _audioRenderers.values) {
      try {
        renderer.srcObject = null;
        await renderer.dispose();
      } catch (e) {
        debugPrint('LiveStream: Renderer disposal error during dispose (ignored): $e');
      }
    }
    ...
    await _audioRendererController.close();
  }
}
